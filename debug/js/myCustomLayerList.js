function myCustomLayerList() {
    ...
    for (var i = 0; i < mapLayers.length; i++) {
        ...
        listImg.src = "assets/images/"+layer.title+".jpg";//aerial_topo.png";
        listImg.width = "60";
        ...
        if (layer.title === "Game Species") {
            ...
            combo.menuPlacement = "header-menu-actions";//"header-actions-end";
            combo.label = "Change Game Species";
            combo.placeholder = "Change game species";
            combo.selectionDisplay = "single";
            combo.clearDisabled = true;
            // add alphabetized list of species to combo
            var speciesArr = layer.sublayers.items;
            speciesArr.sort(function (a, b) {
            ...
            combo.addEventListener("calciteComboboxItemChange", function (event) {
                ...
                event.target.parentElement.parentElement.querySelector("iframe").src = "layer-desc/"+gameSpecies+".html";
            });
            ...
            printBtn.addEventListener("click", function (event) {
                window.open("layer-desc/" + gameSpecies + ".html", "_blank");
            });
            ...
            //iframe.slot = "message";
            iframe.src = "layer-desc/" + gameSpecies + ".html";
        ...
        // Not radio buttons (Game Species)
            ...
            block.appendChild(printBtn);
            // add description file in an iframe
            var iframe = document.createElement("iframe");
            iframe.slot = "message";
            iframe.src = "layer-desc/" + layer.title + ".html";
            iframe.style.border = "none";
            iframe.style.margin = "-12px";
        ...
        //document.getElementsByClassName("esri-ui-top-right")[0].appendChild(sublayerDialog);

        view.ui.add(sublayerDialog,"top-right"); // Does not work!!!!! makes dialogs the entire screen size and prohibits click events!!!!!!
        ...
        /* view.whenLayerView(layer)
         .then(function(layerView) {
             ...
             // search for listItem
             const listItems = tab1.children[0].children;
             for(var i=0;i<listItems.length;i++){
                 if (listItems[i].heading ===layerView.layer.title){
                     listItems[i].removeChild(listItems[i].children[1]);
                     break;
                 }
             }
         ...
         .catch(function(error) {
             ...
             var status;
             for(var i=0;i<listItems.length;i++){
                 status = listItems[i].children[1];
                 if (listItems[i].heading ===layer.title){
                     status.style.visibility = "hidden";
                     break;
                 }
             }
             setTimeout(function(){
                 status.style.visibility = "visible"; // not loaded icon
